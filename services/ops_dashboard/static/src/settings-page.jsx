...
function SettingsPage({ theme, setTheme, t, setTweak }) {
  ...
  function RadioBtn({ value, current, onClick, label }) {
  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 24px 48px" }}>
      ...
      <Section title="表示">
        <Row label="テーマ" hint="ライト / ダーク モードを切り替えます">
          <RadioBtn value="light" current={t.theme || "light"} label="Light"
                    onClick={() => { setTweak("theme", "light"); setTheme("light"); }} />
          <RadioBtn value="dark"  current={t.theme || "light"} label="Dark"
                    onClick={() => { setTweak("theme", "dark");  setTheme("dark");  }} />
        </Row>
        <Row label="密度" hint="行の高さとスペーシングを調整します">
          <RadioBtn value="comfortable" current={t.density || "comfortable"} label="Comfortable"
                    onClick={() => setTweak("density", "comfortable")} />
          <RadioBtn value="compact"     current={t.density || "comfortable"} label="Compact"
                    onClick={() => setTweak("density", "compact")} />
        </Row>
      </Section>

      <Section title="主ビュー">
        <Row label="FlowRow 構成" hint="業務フロー一覧の表示レイアウト">
          <RadioBtn value="chain"  current={t.primary_view || "chain"}  label="Chain"
                    onClick={() => setTweak("primary_view", "chain")} />
          <RadioBtn value="kanban" current={t.primary_view || "chain"} label="Kanban"
